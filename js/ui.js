...
const UI = (() => {
  ...
  function _showStepBlockedMsg(missing) {
    ...
    if (!toast) {
      ...
      toast.style.cssText = 'position:fixed;top:18px;left:50%;transform:translateX(-50%);z-index:9999;background:var(--error,#C42B2B);color:#fff;padding:10px 20px;border-radius:8px;font-size:14px;font-weight:600;box-shadow:0 4px 16px rgba(0,0,0,.25);pointer-events:none;transition:opacity .3s';
        ...
        <div class="card">
          ...
          <div class="btn-row" style="margin-top:10px;flex-wrap:wrap">
            ...
            <select id="nt" aria-label="Select tier for new criterion" style="width:130px">
              <option value="somatic">Practical (Somatic)</option>
              <option value="psychic">Experiential (Psychic)</option>
              <option value="pneumatic">Purpose (Pneumatic)</option>
            </select>
  ...
  function _critListHTML() {
    ...
    // Descriptive tier headings for the setup page criteria list
    const TIER_HEADINGS = {
      somatic:   'Practical Considerations (Somatic)',
      psychic:   'Personal / Experiential Considerations (Psychic)',
      pneumatic: 'Long-Term / Purpose Considerations (Pneumatic)',
    };
    ...
    State.TIERS.forEach(t=>{
      ...
      html+=`<div style="margin-bottom:6px;font-family:var(--font-mono);font-size:10px;
          text-transform:uppercase;letter-spacing:.06em;color:${col};
          padding:5px 8px 4px;border-left:3px solid ${col};background:${col}10;
          border-radius:0 4px 4px 0;font-weight:700"
        data-tip="${_TIP[t]}">${TIER_HEADINGS[t]}</div>`;
      ...
      at.map(t=>`
        <div class="wrow">
          ...
          <input type="range" min="0" max="100" step="1" value="${Math.round((tw[t]||0)*100)}"
            aria-label="${t.charAt(0).toUpperCase()+t.slice(1)} tier weight (current: ${(tw[t]||0).toFixed(2)})"
      ...
      <div class="wrow" style="padding-left:${indentPx}px;${isChild?'border-left:3px solid '+col+'35;margin-left:8px;':''}">
        ...
        <div style="flex:1;display:flex;flex-direction:column;gap:1px">
          <input type="range" min="0" max="100" step="1" value="${Math.round((cw[crit.id]||0)*100)}"
            aria-label="${_esc(crit.name)} weight (current: ${(cw[crit.id]||0).toFixed(2)}, range 0 to 1)"
      ...
      <div style="background:${pageBg};padding:0">
        ...
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px;
          ...
          <button onclick="UI._clearScores()" style="display:inline-flex;align-items:center;gap:6px;
              padding:8px 16px;background:transparent;color:#C42B2B;
              border:1.5px solid #C42B2B;border-radius:7px;
              font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;
        ...
        <div id="score-validation-error" style="display:none;margin-bottom:14px;padding:12px 16px;
            background:rgba(217,54,54,.12);border:1px solid #C42B2B;
            border-left:4px solid #C42B2B;border-radius:6px;
            font-size:13px;color:#C42B2B;line-height:1.6"></div>
        <!-- Tier cards -->
        <div>${tiersHTML}</div>
      </div>
              ...
              ${leaves.map(c=>{
                ...
                return `<div style="margin-bottom:10px">
                  ...
                  <input type="range" min="0" max="1" step="0.01" value="${cw.toFixed(2)}"
                    aria-label="Sensitivity weight for ${_esc(c.name)} (current: ${cw.toFixed(2)}, range 0 to 1)"
  ...
  function _heatMapSVG() {
    ...
    svg += `<rect x="${pl+140}" y="${lgY}" width="14" height="14" fill="#A85200" rx="2"/>`;
  ...
  function _radarHTML() {
    ...
    const ALT_COLORS = ['#1A5276','#A85200','#1A7A6E','#0E6B8C','#8C3D00','#2E5E8E','#7A4A1E','#005050','#3A3A7A','#2C1A5C'];
    ...
    const warnMsg   = activeCnt >= MAX_POLYS
      ? `<p style="text-align:center;font-size:10px;color:var(--error,#C42B2B);font-family:var(--font-mono);margin-top:4px">Max ${MAX_POLYS} alternatives shown at once for readability.</p>`
      : '';
